...
def replacequoted(b):
  ...
  #r = re.compile(r'(?<!\\)\[raw((?:\{.*?\}){,1})]', re.M + re.S)
...
def replaceimages(b):
  # works with [img{width}{height}{alttext}{caption} location caption].
  r = re.compile(r'(?<!\\)\[img((?:\{.*?\}){,4})\s(.*?)(?:\s(.*?))?(?<!\\)\]',
           re.M + re.S)
  m = r.search(b)
  ...
  while m:
    ...
    if m1[2]:
      ...
      bits.append(r'alt=\"\"')
    bits.append(r'class=\"center\"')
    b = b[:m.start()] + r'<img %s />' % " ".join(bits) + r'<figcaption>%s</figcaption>' % quote(m1[3]) + r'<br />' + b[m.end():]
...
def br(b, f, tableblock=False):
  ...
  b += u'\n\n\n\n\n' #a hacky way to prevent the regex consuming the last char before new line char
